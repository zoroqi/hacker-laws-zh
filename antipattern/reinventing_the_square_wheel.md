...
在实际应用层面，从事软件开发的工作者也时常在做“重造轮子”的事，不过这并非全无价值，例如用来回避软件许可问题、第三方模组或零件的技术限制。例如要对资料排序时，最常使用的快速排序法，虽然广为人知、而且已建置于 C++ 与 Java 的通用函式库中，但网页用途的 Javascript 在某些实作版本（如网页浏览器）却不一定包括此算法，又或是效能因素考量，此时可能须要“重造轮子”──重新编写快速排序法。
